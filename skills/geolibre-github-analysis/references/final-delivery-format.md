...
6. 若六項中的某項未產出、無法開啟或尚未驗證，**禁止使用本文件的最終成果範本**；必須先回到修復流程，解決後完整重跑並重新驗收。只有使用者本人必須完成的外部授權仍未解除時，才可另外回報「尚未完成」狀態，但不得稱為最終成果。
...
1. 「直接在你的 GeoLibre 開啟這次分析」必須真的是已載入本次完整圖資專案的可用入口，不能只給 JSON／GeoJSON 檔案下載網址。
2. 任一固定成果缺失、失效、未同步新版或未通過 QA，都視為整體驗收失敗。
